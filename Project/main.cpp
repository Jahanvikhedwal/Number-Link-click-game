 /*  Description: Code printing valid game puzzle along with its solution for the game of flow.
    Creator: Jahanvi Khedwal (jahanvikhedwal.nitrr@gmail.com) */
#include<stdio.h>
#include<stdlib.h>
#include<time.h>
...
/*    Name: initialize_board()
      ...
      Output: --void--
      Description: Takes a table(2-D array) of pointers and initializes it. */
void initialize_board(node ***arr, int n)
{
	int i, j;
	for (i=0;i<n;i++){
		for (j=0;j<n;j++){
			...
			node_pointer->path_number = 0;
			node_pointer->endpoint = 0;
			arr[i][j] = node_pointer;
		}
	}
}
/***********************/
...
/*    Name: valid_starting_neighbours()
      Input: size of array row/column, 2D-array of pointers
      Output: returns the encrypted/coded values of k1 and k2 or -1 if fails.
      Description: Finds the valid starting points of the flow. */
int valid_starting_neighbours(int n, node ***arr)
{
...
/*    Name: check_neighbours()
      ...
      Description: Checks the neighbouring 4 values( Two horizontally and two vertically and find if there are more than 1 neighbours of the same set.

*/
int check_neighbours(int k1, int k2, int n, node ***arr)
{
	int i= k2/n;
	int j= k2%n;
	int ii= k1/n;
...
int valid_next(int k, int n, node ***arr)
{
	...
	switch(kk)
	{
		case 0: i2= i1-1;
				...
				if(i2>=0 && i2<n && j2<n){
					if(arr[i2][j2]->path_number==0){
						stat= check_neighbours(k, (n*i2 + j2),n,arr);
						//printf("%d\n",stat);
						if(stat){
							flag=1;
							break;
...
int addpath(node ***arr, int n, int ptno)
{
	...
	int a,b,k1,k2;
	int i1,j1,i2,j2;
	k2= valid_starting_neighbours( n, arr);
